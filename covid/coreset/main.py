...
def main(args):
    ...
    for iiter in range(args.global_iteration1): 
        ...
        if iiter!=args.global_iteration1-1:
            for k in range(args.num_clients):
                arg = solver[k].sample_for_labeling(R_task_model[k],R_FC1[k], R_FC2[k],unlabeled_dataloader[k],querry_dataloader[k],train_dataset[k],unlabeled_indices[k],labeled_indices[k],len(labeled_indices[k]))
