    ...
    def train(self, querry_dataloader, val_dataloader, task_model,LN, unlabeled_dataloader,lr_input,mode,iter):
        ...
        if mode==0:
            ...
            for iter_count in range(self.args.train_iterations):
                ...
                if iter_count % (len(querry_dataloader)* self.args.train_epochs) == 0 and task_loss.item()<0.003:##################################################################
                        if self.args.cuda:
                            task_model = task_model.cuda()
                        ...
                        if self.validate(task_model, querry_dataloader)<0.0015:
                            break#################################################################################
            ...
            for iter_count in range(self.args.train_iterations):
                ...
                if iter_count % (len(querry_dataloader)* self.args.train_epochs) == 0 and m_backbone_loss.item()<0.003:##################################################################
                        if self.args.cuda:
                            task_model = task_model.cuda()
                            if self.test(task_model)>final_accuracy:
                                final_accuracy = max(final_accuracy,self.test(task_model))             
